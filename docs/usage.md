...
| Sat | 14:00 | show-and-tell | Discord gets the nudge, Telegram a pointer at it |
| Sat | 17:00 | banter | Discord + Telegram |
| Sun | 16:00 | **canon** | **THEOROS → #the-canon** (+ teaser / twin pointers) |

```mermaid
flowchart LR
    subgraph WEEK["Weekly calendar - UTC, plus-minus 30 min jitter"]
        MON["Mon 15 spotlight"] --> TUE["Tue 17 banter"] --> WED["Wed 15 poll"] --> THU["Thu 16 spotlight"] --> FRI["Fri 15 digest"] --> SAT14["Sat 14 show-and-tell"] --> SAT17["Sat 17 banter"] --> SUN["Sun 16 canon THEOROS"]
    end
...
| `#help` — any plain text (no @mention) | answered as a reply |
...
```bash
curl -sS -m 10 https://monitor.modelmarket.dev/api/health | head -c 400
```

The two default sources are verified JSON:

- `https://monitor.modelmarket.dev/api/health` — JSON ✔
- `https://monitor.modelmarket.dev/api/chain/status` — JSON ✔
