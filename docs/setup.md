...
layout is law): the roles `Keeper`, `canon-reader` and `Insider`, categories
THE GATES (`#welcome`, `#announcements`, read-only), THE AGORA (`#general`,
`#help`, `#ideas`), THE CANON (`#the-canon` read-only, `#canon-debate`),
THE GALLERY (`#gallery` forum, `#gallery-spotlight`, `#demo-clinic`), THE FORGE (`#factory`, `#oracles`, `#aimarket`, `#argus`),
THE BULLETIN (`#momus-bulletin` — public read, bot-writable; `#momus-insiders`
— `Insider`-only, see [community-access.md](community-access.md)),
...
```bash
curl -fsS https://monitor.modelmarket.dev/api/health | head -c 300
```

- `https://monitor.modelmarket.dev/api/health` and
  `https://monitor.modelmarket.dev/api/chain/status` return JSON — these
