...
DIOSCURI is one Node.js process wearing two faces: CASTOR answers on Telegram,
POLLUX answers on Discord. Both personas share one knowledge base (MNEMOSYNE),
one injection firewall (AEGIS), one LLM client chain, one audit trail and one
content calendar (THEOXENIA). This document maps the modules, explains how they
are wired together, and walks through the five data flows that matter. The
security rationale behind several of these choices lives in
[docs/security.md](security.md); this document covers structure, not threat
analysis.
...
| THEOXENIA | `src/theoxenia/` | Content engine: weekly slot calendar (spotlight / banter / poll / digest / show-and-tell), manual topic queue, KB-grounded generation, quiet hours and daily post caps, post guard, fan-out to adapters. State in `/data/theoxenia.json`; queue in `/data/content-queue.json`. |
