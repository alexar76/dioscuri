 ...
 *  - a policied channel is born already denied (creationOverwrites) — the one
 *    executor seam that is pure, and the one whose failure cannot be retried;
...
import { creationOverwrites } from "../src/provision/discord.js";
...
describe("planProvision", () => {
  it("plans the full structure for an empty guild, in dependency order", () => {
    ...
    expect(permFor(PANTHEON_ARTS_CHANNEL)?.overwrites).toBe("readonly");
    expect(permFor(PANTHEON_TALES_CHANNEL)?.overwrites).toBe("readonly");
    expect(opsOf(plan, "createCategory").some((s) => s.name === PANTHEON_CATEGORY)).toBe(true);
    // The bulletin category must stay OPEN: a policy here would hide the public
    // #momus-bulletin along with the gated write-up channel beside it.
    expect(permFor(BULLETIN_CATEGORY)).toBeUndefined();
    expect(permFor(MOMUS_BULLETIN_CHANNEL)?.overwrites).toBe("readonly");
    expect(permFor(MOMUS_INSIDERS_CHANNEL)?.overwrites).toBe("insidersonly");
  ...
  it("creates only a missing role when everything else exists", () => {
    ...
    expect(plan).toEqual([
      ...
      { op: "createRole", name: INSIDER_ROLE },
    ]);
  });
...
// ---------------------------------------------------------------------------
// creationOverwrites — the policy a channel is BORN with
// ---------------------------------------------------------------------------

describe("creationOverwrites", () => {
  const EVERYONE = "guild-id-as-everyone";
  const BOT = "bot-id";
  const forEveryone = (out: ReturnType<typeof creationOverwrites>) =>
    out?.find((o) => o.id === EVERYONE);
  const forBot = (out: ReturnType<typeof creationOverwrites>) => out?.find((o) => o.id === BOT);

  it("hides an insidersonly channel from @everyone in the create call itself", () => {
    // The gated channel lives in a category with NO policy (so the public
    // bulletin beside it stays public), so nothing else would hide it: if the
    // deny only arrived in the setPermissions step, a failure there would leave
    // #momus-insiders world-readable forever — the next boot adopts it and
    // plans no policy for it.
    const out = creationOverwrites("insidersonly", EVERYONE, BOT);
    expect(forEveryone(out)?.deny).toEqual([PermissionFlagsBits.ViewChannel]);
    expect(forEveryone(out)?.allow).toBeUndefined();
  });

  it("keeps a readonly channel VISIBLE and only takes away posting", () => {
    // #momus-bulletin is public by design; denying ViewChannel here would be
    // the exact failure the public bulletin exists to rule out.
    const out = creationOverwrites("readonly", EVERYONE, BOT);
    expect(forEveryone(out)?.deny).toEqual([PermissionFlagsBits.SendMessages]);
    expect(forEveryone(out)?.deny).not.toContain(PermissionFlagsBits.ViewChannel);
  });

  it("hides a modonly channel the same way", () => {
    expect(forEveryone(creationOverwrites("modonly", EVERYONE, BOT))?.deny).toEqual([
      PermissionFlagsBits.ViewChannel,
    ]);
  });

  it("lets the bot back in, so it never locks itself out of a channel it posts in", () => {
    for (const policy of ["readonly", "modonly", "insidersonly"] as const) {
      const bot = forBot(creationOverwrites(policy, EVERYONE, BOT));
      expect(bot?.allow).toEqual([PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages]);
      expect(bot?.deny).toBeUndefined();
    }
  });

  it("grants NO role at birth — the role grant is the setPermissions step's job", () => {
    // Only @everyone and the bot may appear. A role granted here would skip the
    // missing-role warning that applyOverwrites raises.
    for (const policy of ["readonly", "modonly", "insidersonly"] as const) {
      const ids = (creationOverwrites(policy, EVERYONE, BOT) ?? []).map((o) => o.id);
      expect(ids).toEqual([EVERYONE, BOT]);
    }
  });

  it("still denies @everyone when the bot id is unknown", () => {
    const out = creationOverwrites("insidersonly", EVERYONE, undefined);
    expect(out).toEqual([{ id: EVERYONE, deny: [PermissionFlagsBits.ViewChannel] }]);
  });

  it("leaves an unpolicied channel alone (inherits its category, as before)", () => {
    expect(creationOverwrites(undefined, EVERYONE, BOT)).toBeUndefined();
  });

  it("covers every policy the structure actually declares", () => {
    // A new OverwritePolicy that nobody taught this function would silently
    // create an OPEN channel; this fails the moment one is added.
    const declared = new Set<string>();
    for (const cat of DESIRED_STRUCTURE.categories) {
      if (cat.overwrites) declared.add(cat.overwrites);
      for (const ch of cat.channels) if (ch.overwrites) declared.add(ch.overwrites);
    }
    for (const policy of declared) {
      const out = creationOverwrites(policy as "readonly" | "modonly" | "insidersonly", EVERYONE, BOT);
      expect(forEveryone(out)?.deny?.length).toBeGreaterThan(0);
    }
  });
});
