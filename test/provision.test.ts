...
const CREATE_OPS = new Set(["createRole", "createCategory", "createText", "createVoice"]);
...
describe("planProvision", () => {
  it("plans the full structure for an empty guild, in dependency order", () => {
    ...
    expect(opsOf(plan, "createText").length + opsOf(plan, "createVoice").length).toBe(desiredChannelCount);
    ...
    const firstChannel = idx((s) => s.op === "createText" || s.op === "createVoice");
    ...
    for (const step of plan) {
      if (step.op === "createText" || step.op === "createVoice") {
  ...
  it("creates only a missing role when everything else exists", () => {
    ...
    const plan = planProvision(existing);
    expect(plan).toEqual([{ op: "createRole", name: KEEPER_ROLE }]);
  });
