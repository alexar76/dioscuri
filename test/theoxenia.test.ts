...
import { Theoxenia, earliestSlotKinds, type ContentTuning, type TheoxeniaOpts } from "../src/theoxenia/engine.js";
...
// ---------------------------------------------------------------------------
// Scheduling — co-timed slots must not starve
// ---------------------------------------------------------------------------

describe("scheduling", () => {
  it("earliestSlotKinds returns every kind co-timed at the next occurrence", () => {
    const beforeSun16 = Date.UTC(2024, 0, 14, 15, 0, 0);
    const picked = earliestSlotKinds(
      [
        { kind: "show-and-tell", day: "sun", hourUtc: 16 },
        { kind: "canon", day: "sun", hourUtc: 16 },
        { kind: "banter", day: "mon", hourUtc: 15 },
      ],
      beforeSun16,
    );
    expect(picked).not.toBeNull();
    expect(picked!.kinds).toEqual(["show-and-tell", "canon"]);
    expect(picked!.at).toBe(Date.UTC(2024, 0, 14, 16, 0, 0));
  });
});
...
describe("spotlight screenshots", () => {
  it("attaches a README demo screenshot when topic matches a repo", async () => {
    ...
    const h = harness({
      ...
      kbDemo: { "alien-monitor": "https://monitor.modelmarket.dev/" },
