...
export class MnemosyneKB implements Mnemosyne {
  ...
  private processRelease(
    ...
    // Seeding an empty store marks tags as seen WITHOUT announcing them — that is the point of
    // the first pass, so the mark belongs here.
    if (firstSeed) {
    ...
    // Marked seen only now that the event EXISTS. Marking before this point meant a tag could be
    // burned without ever being announced: publish `v1.2.3`, let one poll see it, delete it — and
    // the real `v1.2.3` is suppressed for good, because the id is already in the store.
    this.store.markReleaseSeen(seenId);
  }
