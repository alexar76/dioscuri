...
// ---------------------------------------------------------------------------
// Release-note plain text (Telegram + syndication — no raw GitHub markdown)
// ---------------------------------------------------------------------------

/** Flatten release-note markdown into plain one-line text. */
...
/** First sentence of a summary, single-line, capped. */
export function firstSentence(text: string, maxChars = 120): string {
  ...
  if (m !== null) {
    ...
    const clause = (t.split(/\s+[-–—]\s+/)[0] ?? t).trim();
    const words = clause.split(/\s+/);
    s = words.length > 12 ? words.slice(0, 12).join(" ") : clause;
  }
  return s.length > maxChars ? s.slice(0, maxChars - 1).trimEnd() + "…" : s;
}

/** Short plain blurb for release announcements (no ## / ** / code fences). */
export function releaseBlurb(summary: string, maxChars = 120): string {
  return firstSentence(stripMarkdown(summary), maxChars);
}
