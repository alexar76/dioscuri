    ...
    showcase: z
      .object({
        ...
        sources: z
          ...
          .default([
            { name: "alien-monitor", url: "https://monitor.modelmarket.dev/api/health", kind: "json" },
            { name: "monitor-chain", url: "https://monitor.modelmarket.dev/api/chain/status", kind: "json" },
          ]),
      })
      .default({}),
    content: z
      ...
      .default({}),
    /**
     * MOMUS security bulletin → the public #momus-bulletin channel.
     *
     * Off by default, and it stays off unless BOTH a pinned publisher key and a
     * channel resolve: an advisory posted under our bot without a verified
     * signature would let whoever controls the network path publish accusations
     * in our name. See src/bulletin/index.ts.
     */
    bulletin: z
      .object({
        enabled: z.boolean().default(false),
        /** MOMUS's signed index. */
        indexUrl: z.string().url().default("https://momus.modelmarket.dev/bulletin"),
        /**
         * PINNED Ed25519 publisher key, hex SPKI DER — MOMUS serves it as
         * `index_pubkey_spki_hex` on /bulletin, but the point of a pin is that it
         * comes from config, never from the document being verified. Empty ⇒ the
         * publisher refuses to start. `MOMUS_BULLETIN_PUBKEY` overrides.
         */
        publicKeyHex: z.string().default(""),
        /** Minutes between polls (floor 5, enforced by the publisher). */
        pollIntervalMin: z.number().int().min(5).max(24 * 60).default(30),
        /** Ceiling per cycle, so a cold start drips instead of flooding. */
        maxPostsPerRun: z.number().int().min(1).max(20).default(5),
        /** Freshness window for the signed snapshot (hours). */
        maxAgeHours: z.number().int().min(1).max(168).default(24),
        /**
         * Insiders' write-up base URL. Empty (default) renders no write-up line —
         * the advisory itself is public either way.
         */
        writeupBaseUrl: z.string().default(""),
      })
      .default({}),
  })
  .default({});
...
export interface DioscuriConfig {
  ...
  discord: {
    ...
    /** Empty = auto-created/discovered (#pantheon-arts). */
    pantheonArtsChannelId: string;
    /** Empty = auto-created/discovered (#pantheon-tales). */
    ...
    /** Empty = auto-created/discovered (#momus-bulletin). */
...
export function loadConfig(tuningPath = env("DIOSCURI_CONFIG", "dioscuri.config.json")): DioscuriConfig {
  ...
  return {
    ...
    discord: {
      ...
      pantheonArtsChannelId: env("DISCORD_PANTHEON_ARTS_CHANNEL_ID"),
      pantheonTalesChannelId: env("DISCORD_PANTHEON_TALES_CHANNEL_ID"),
      canonChannelId: env("DISCORD_CANON_CHANNEL_ID"),
      generalChannelId: env("DISCORD_GENERAL_CHANNEL_ID"),
      bulletinChannelId: env("DISCORD_BULLETIN_CHANNEL_ID"),
