...
import { enqueueTheorosFromCanonPost, isHeliosSyndicationEnabled } from "../helios/enqueue.js";
import { banter, canon, digest, poll, showAndTell, spotlight, type GeneratorDeps, type PollResult } from "./generator.js";
import { consumeQueue, topicHash, TheoxeniaState } from "./state.js";
import type { CanonDiscordPost } from "../types.js";
...
export function nextOccurrence(slot: ContentSlot, nowMs: number): number {
...
/**
 * Earliest upcoming slot time and every kind that shares it.
 * Co-timed slots (same day+hour) must all be returned — otherwise the first
 * kind in config starves the rest forever (show-and-tell vs canon on Sun 16).
 */
export function earliestSlotKinds(
  slots: readonly ContentSlot[],
  nowMs: number,
): { at: number; kinds: ContentKind[] } | null {
  ...
  for (const slot of slots) {
    ...
    } else if (at === bestAt && !kinds.includes(slot.kind)) {
      kinds.push(slot.kind);
    }
  }
  if (bestAt === null || kinds.length === 0) return null;
  return { at: bestAt, kinds };
}
...
export class Theoxenia implements ContentEngine {
  ...
  private scheduleNext(): void {
    ...
    const picked = earliestSlotKinds(slots, nowMs);
    if (picked === null) return;
    const jitter = (this.random() * 2 - 1) * SLOT_JITTER_MS;
    const delay = Math.max(MIN_DELAY_MS, picked.at - nowMs + jitter);
    const kindsAtBest = picked.kinds;
    this.log.info("theoxenia slot scheduled", {
      kinds: kindsAtBest,
    ...
    this.mainTimer = this.setTimeoutFn(() => {
      this.mainTimer = null;
      void (async () => {
        for (const kind of kindsAtBest) {
          await this.runSlot(kind);
        }
  ...
  private async runBanter(): Promise<void> {
    ...
    if (memes && memeBudget > 0 && this.state.memesThisWeek() < memeBudget && adapterA.announceImage) {
      try {
        ...
        if (adapterA.announcePantheonArt) {
          await adapterA.announcePantheonArt(image, joke.setup);
        } else {
    ...
    if (!setupDelivered) {
      if (adapterA.announcePantheonTale) {
        await adapterA.announcePantheonTale(joke.setup);
      } else {
        await adapterA.announce(joke.setup);
      }
    }
  ...
  private async deliverPunchline(adapter: ChannelAdapter, platform: Platform, text: string): Promise<void> {
    try {
      if (adapter.announcePantheonTale) {
        await adapter.announcePantheonTale(text);
      } else {
  ...
  private async runShowAndTell(): Promise<void> {
    ...
    if (dc) {
      if (dc.announceGallery) {
        await dc.announceGallery(nudge);
      } else {
  ...
  /** THEOROS column: Discord #the-canon + teaser in #announcements + twin pointers. */
  private async runCanon(): Promise<void> {
    const dc = this.available("discord");
    if (!dc) return;
    if (!dc.announceCanon) {
      this.log.info("theoxenia canon skipped — announceCanon not configured");
      return;
    }
    const topic = await this.pickTopic("canon");
    if (topic === null) return;

    const chapterIdx = this.state.getRotation("canon-chapter");
    this.state.setRotation("canon-chapter", chapterIdx + 1);

    const piece = await canon(this.genDeps(), topic, chapterIdx);

    await dc.announceCanon(piece.canonPost);
    await dc.announce(piece.teaser);
    if (dc.announceGeneral) {
      await dc.announceGeneral(piece.polluxPointer);
      await this.recordPost("discord", "canon", piece.polluxPointer);
    }
    const tg = this.available("telegram");
    if (tg) {
      await tg.announce(piece.castorPointer);
      await this.recordPost("telegram", "canon", piece.castorPointer);
    }

    // HELIOS: order a Short from Theoros's own column (no CALLIOPE rewrite).
    this.enqueueTheorosHelios(piece.canonPost, chapterIdx, topic);

    this.state.recordHash(topicHash("canon", topic));
    await this.recordPost("discord", "canon", `${piece.canonPost.chapterLabel}\n${piece.canonPost.body}`);
    await this.recordPost("discord", "canon", piece.teaser);
    await this.state.save();
  }

  /** Fail-soft Helios syndication for THEOROS — same gate as release-shorts. */
  private enqueueTheorosHelios(post: CanonDiscordPost, chapterIdx: number, topic: string): void {
    if (!isHeliosSyndicationEnabled()) return;
    const queuePath = process.env.HELIOS_QUEUE_PATH?.trim();
    if (!queuePath) return;
    const chapterKey = `${chapterIdx}:${topicHash("canon", topic)}`;
    enqueueTheorosFromCanonPost(post, chapterKey, {
      log: this.log.child("helios"),
      queuePath,
    });
  }
}
