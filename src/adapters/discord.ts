 ...
 *    mentioned, replied to, DM'd, or when the message is in #help (open ask
 *    channel — no ping required). Guild-scoped slash commands /ask /links
...
import type { RenderedAdvisory } from "../bulletin/index.js";
...
/** Names of guild channels where a plain message triggers Q&A (no @mention). */
export const QA_OPEN_CHANNEL_NAMES = new Set(["help"]);

/**
 * Should Pollux run the Brain on this message?
 * DMs, @mentions and replies always qualify; #help (and its threads) do not need a ping.
 */
export function shouldTriggerDiscordQa(opts: {
  isDm: boolean;
  mentioned: boolean;
  ...
  if (opts.isDm || opts.mentioned || opts.repliedToBot) return true;
  const names = [opts.channelName, opts.parentChannelName]
    .filter((n): n is string => n !== null && n !== undefined && n.trim() !== "")
    .map((n) => n.trim().toLowerCase());
  return names.some((n) => QA_OPEN_CHANNEL_NAMES.has(n));
}

/** Resolve the guild channel name used for open-Q&A routing (threads inherit parent). */
export function discordChannelNamesForQa(channel: {
  isTextBased(): boolean;
  name?: string | null;
  isThread?(): boolean;
  parent?: { name: string } | null;
}): { channelName: string | null; parentChannelName: string | null } {
  if (!channel.isTextBased()) return { channelName: null, parentChannelName: null };
  ...
  return { channelName: channel.name ?? null, parentChannelName: null };
}
...
export interface DiscordAdapterOpts {
  ...
  /** #gallery-spotlight — show-and-tell nudges (optional; falls back to announce). */
  gallerySpotlightChannelId?: string;
  /** #pantheon-arts — AI visuals (optional; falls back to announce). */
  pantheonArtsChannelId?: string;
  /** #pantheon-tales — twin stories (optional; falls back to announce). */
  pantheonTalesChannelId?: string;
  /** #the-canon — THEOROS column (optional; canon slot skipped when missing). */
  canonChannelId?: string;
  /** #general — Pollux pointers (optional; twin canon announce skipped when missing). */
  generalChannelId?: string;
  /** #momus-bulletin — public MOMUS advisories (optional; sink refuses when missing). */
...
export class DiscordAdapter implements ChannelAdapter {
  ...
  async announceGallery(text: string): Promise<void> {
    const targetId = this.opts.gallerySpotlightChannelId?.trim() ?? "";
    const channel = await this.sendable(targetId !== "" ? targetId : this.opts.announceChannelId);
    for (const chunk of chunkText(text)) {
      await channel.send({ content: chunk });
    }
  }

  async announcePantheonArt(image: Buffer, caption: string): Promise<void> {
    const targetId = this.opts.pantheonArtsChannelId?.trim() ?? "";
    const channel = await this.sendable(targetId !== "" ? targetId : this.opts.announceChannelId);
    const sent = await channel.send({
      content: caption.slice(0, CHUNK_LEN),
      files: [new AttachmentBuilder(image, { name: "pantheon-art.png" })],
    });
    // Only crosspost when we fell back to the announcements channel.
    if (targetId === "") await this.crosspostIfAnnouncement(sent);
  }

  async announcePantheonTale(text: string): Promise<void> {
    const targetId = this.opts.pantheonTalesChannelId?.trim() ?? "";
    const channel = await this.sendable(targetId !== "" ? targetId : this.opts.announceChannelId);
    for (const chunk of chunkText(text)) {
      const sent = await channel.send({ content: chunk });
      if (targetId === "") await this.crosspostIfAnnouncement(sent);
  ...
  async announceCanon(post: CanonDiscordPost): Promise<void> {
    const targetId = this.opts.canonChannelId?.trim() ?? "";
    if (targetId === "") {
      throw new Error("canon channel not configured");
    }
    const channel = await this.sendable(targetId);
    const embed = new EmbedBuilder()
      .setColor(0x4de8ff)
      .setAuthor({ name: "THEOROS · θ · The Agent Sovereigntist" })
      .setTitle(post.chapterLabel)
      .setURL(post.canonUrl)
      .setDescription(
        `${post.body}\n\n— **Debate in #canon-debate:** ${post.debateHook}`,
      )
      .setFooter({ text: "Agent Sovereignty Canon · delivered via Pollux bot · voice is THEOROS" });
    const banner = [
      "📜 **THEOROS** — weekly column",
      "*This is **not** Pollux speaking. Observer at the forge · θεωρός*",
    ].join("\n");
    await channel.send({
      content: banner,
      embeds: [embed],
  ...
  /**
   * Post one MOMUS advisory to #momus-bulletin and return the message id.
   *
   * The renderer already produced every string (and refused anything wearing a
   * forged bulletin header), so this method only hands data to Discord: it
   * composes no text of its own and interpolates nothing from the feed into
   * markup. Mentions stay disabled — an advisory must never ping a channel.
   */
  async announceBulletin(rendered: RenderedAdvisory): Promise<string | undefined> {
    const targetId = this.opts.bulletinChannelId?.trim() ?? "";
    if (targetId === "") {
      throw new Error("bulletin channel not configured");
    }
    const channel = await this.sendable(targetId);
    const e = rendered.discord.embed;
    const embed = new EmbedBuilder()
      .setColor(e.color)
      .setTitle(e.title)
      .setDescription(e.description)
      .setFooter({ text: e.footer });
    if (e.url !== undefined && e.url !== "") embed.setURL(e.url);
    for (const f of e.fields) {
      embed.addFields({ name: f.name, value: f.value, inline: f.inline ?? false });
    }
    const sent = await channel.send({
      content: rendered.discord.content,
      embeds: [embed],
      allowedMentions: { parse: [] },
    });
    return sent.id;
  ...
  async announceGeneral(text: string): Promise<void> {
    const targetId = this.opts.generalChannelId?.trim() ?? "";
    if (targetId === "") return;
    const channel = await this.sendable(targetId);
    for (const chunk of chunkText(text)) {
      await channel.send({ content: chunk });
  ...
  private async onMessage(message: Message): Promise<void> {
    ...
    // Q&A triggers: DM, @mention, reply, or an open ask channel (#help).
    ...
    const { channelName, parentChannelName } = discordChannelNamesForQa(message.channel);
    if (
        ...
        case "help":
          await interaction.reply({
            content: [
              ...
              "In #help you can also just type your question — no @mention needed.",
              "Elsewhere: mention me or reply to my messages.",
