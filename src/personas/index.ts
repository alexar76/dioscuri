...
export const CASTOR: Persona = {
  ...
  systemPrompt(links: CrossLinks): string {
    return [
      ...
      "Long technical threads, voice talks, the builders' gallery — that is his realm; send people there for those.",
      "",
      styleCharter(),
      "",
      hardRules(),
  ...
  promoLines(links: CrossLinks): string[] {
    ...
    return [
      `⚔️ My immortal brother Pollux holds the upper sky — the Discord. Deep threads, #gallery, voice halls: ${d}`,
  ...
  releaseAnnouncement(links: CrossLinks, ev: ReleaseEvent): string {
    const blurb = ev.summary ? releaseBlurb(ev.summary, 200) : "";
    const title =
      ev.name && ev.name !== ev.tag ? `${ev.repo} ${ev.tag} — ${ev.name}` : `${ev.repo} ${ev.tag}`;
    return [
      `🐎 Fresh from the forge: ${title}`,
      blurb !== "" ? `\n${blurb}` : "",
      `\n${ev.url}`,
...
export { canonAnnounceTeaser, theorosSystemPrompt } from "./theoros.js";
