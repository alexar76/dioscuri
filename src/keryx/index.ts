...
export interface SignalHeroAnnouncement {
  id: string;
  handle: string;
  status: string;
  score: number;
...
export class Keryx {
  ...
  hasSink(name: string): boolean {
    return this.sinks.some((sink) => sink.name === name);
  }

  composeSignalHero(hero: SignalHeroAnnouncement): string {
    const reward = hero.rewards[0]?.replace(/^tier:/, "status ").replaceAll("_", " ") ?? "verified milestone";
    const raw = [
      `✦ SIGNAL HUNT HERO — ${hero.handle}`,
      `${hero.status.replaceAll("_", " ")} · ${hero.score} verified points · ${hero.rounds} rounds`,
      `Unlocked: ${reward}`,
      hero.url,
    ].join("\n");
    return prepareUntrusted(raw, 360)
      .replaceAll("@everyone", "everyone")
      .replaceAll("@here", "here");
  }

  /** Signal Hunt heroes go to X only; Discord is delivered by Pollux directly. */
  async announceSignalHero(hero: SignalHeroAnnouncement): Promise<void> {
    const sink = this.sinks.find((candidate) => candidate.name === "x");
    if (!sink) return;
    const text = this.composeSignalHero(hero);
    try {
      await sink.post(text);
      this.log.info("signal hero posted", { sink: "x", eventId: hero.id });
    } catch (err) {
      this.log.warn("signal hero X post failed — retryable", {
        eventId: hero.id,
        error: err instanceof Error ? err.message : String(err),
      });
      throw err;
