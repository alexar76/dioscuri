...
function firstSentence(text: string): string {
  ...
  if (m !== null) {
    ...
    // Release notes often lack periods — take the title clause, not the whole wall.
    const clause = (t.split(/\s+[-–—]\s+/)[0] ?? t).trim();
    const words = clause.split(/\s+/);
    s = words.length > 12 ? words.slice(0, 12).join(" ") : clause;
  }
  return s.length > SUMMARY_MAX_CHARS ? s.slice(0, SUMMARY_MAX_CHARS - 1).trimEnd() + "…" : s;
}

function releaseBlurb(summary: string): string {
  return firstSentence(stripMarkdown(summary));
}
...
export class Keryx {
  ...
  composeRelease(ev: ReleaseEvent): string {
    const summary = releaseBlurb(ev.summary);
    const head =
      summary !== ""
