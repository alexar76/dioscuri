 ...
 *              ViewChannel is left alone — readonly means PUBLIC to read.
 *  - modonly:  @everyone denied ViewChannel; Keeper role + bot allowed.
 *  - insidersonly: @everyone denied ViewChannel; Insider role + bot allowed.
 *              The deny is applied BEFORE the role is granted access, so a
 *              missing role leaves the channel shut rather than open.
 * Every policy's DENY half is also baked into the create call itself
 * (creationOverwrites) so that a channel is never briefly public — see that
 * function for why a retry cannot repair it afterwards.
 *
...
import {
  ...
  AGORA_CATEGORY,
  PANTHEON_CATEGORY,
  PANTHEON_ARTS_CHANNEL,
  PANTHEON_TALES_CHANNEL,
  findExistingChannel,
  planProvision,
  type DesiredChannel,
  type ExistingEntity,
...
/** Channel kinds the provisioner manages (threads/stages are ignored). */
type ProvisionableChannel = CategoryChannel | TextChannel | NewsChannel | VoiceChannel | ForumChannel;
...
export interface ProvisionDiscordResult {
  ...
  /** #gallery-spotlight — show-and-tell nudges and weekly picks. */
  gallerySpotlightChannelId: string;
  /** #pantheon-arts — AI visuals / declarative art. */
  pantheonArtsChannelId: string;
  /** #pantheon-tales — twin stories and banter. */
  pantheonTalesChannelId: string;
  /** #the-canon — THEOROS weekly column. */
  canonChannelId: string;
  /** #general — Pollux pointers (canon launch, cross-promo). */
  generalChannelId: string;
  /**
   * #momus-bulletin — where verified MOMUS advisories are posted. Discovered
   * here so an operator does not have to paste a channel id for the public
   * half of the bulletin to work; a pinned DISCORD_BULLETIN_CHANNEL_ID still
   * wins, the same way the other ids are treated in src/index.ts.
   */
...
/**
 * The half of a policy that must be true from the channel's FIRST MOMENT: the
 * deny, plus the bot's own way back in. The role grant is deliberately absent —
 * applyOverwrites adds it in the setPermissions step that follows.
 *
 * Why this exists at all, when that step applies the same policy seconds later:
 * a new channel inherits its CATEGORY's permissions, and 🛰 THE BULLETIN carries
 * none on purpose so the public advisory channel stays public. #momus-insiders
 * is therefore born world-readable, and it STAYS that way if the setPermissions
 * call fails — the next boot adopts the existing channel and plans no policy for
 * it (planProvision only permissions what it creates), so nothing ever heals it.
 * The same reasoning covers readonly: a #momus-bulletin that is briefly writable
 * by @everyone is a security bulletin any member can forge a post into, in the
 * one channel whose entire authority is that only the bot speaks there.
 *
 * Denying at birth and granting afterwards means a half-applied policy fails
 * SHUT: worst case an operator has to hand out access, not chase down a leak.
 *
 * Accepted trade: passing explicit overwrites also unsyncs the channel from its
 * category, so a POLICIED channel created inside a category an admin had
 * customised no longer inherits that customisation. applyOverwrites unsynced it
 * one call later anyway; what changes is that the admin's category overwrites
 * are not copied down first. Unpolicied channels are untouched and still sync.
 */
export function creationOverwrites(
  policy: OverwritePolicy | undefined,
  everyoneId: string,
  botId: string | undefined,
): OverwriteData[] | undefined {
  if (policy === undefined) return undefined;
  // readonly stays VISIBLE to @everyone — it only loses the right to post.
  const deny =
    policy === "readonly" ? [PermissionFlagsBits.SendMessages] : [PermissionFlagsBits.ViewChannel];
  const overwrites: OverwriteData[] = [{ id: everyoneId, deny }];
  if (botId !== undefined) {
    // Without this the bot locks itself out of a channel it has to post in, and
    // a hidden channel is exactly where it cannot ask for help.
    overwrites.push({
      id: botId,
...
async function applyOverwrites(
  ...
  if (policy === "readonly") {
    // ViewChannel is untouched: readonly means "everyone reads, the bot writes".
    // Denying it here would quietly turn #momus-bulletin into a private channel,
    // which is the exact failure the public bulletin exists to rule out.
  ...
  // modonly / insidersonly: same shape, different key holder — @everyone loses
  // sight of the channel and exactly ONE role gets it back. The deny is applied
  // BEFORE the role lookup on purpose: if the role turns out to be missing, a
  // shut channel is the safe end state and an open one is not.
  const keyHolder = policy === "modonly" ? KEEPER_ROLE : INSIDER_ROLE;
  await target.permissionOverwrites.edit(everyone, { ViewChannel: false }, { reason: PROVISION_REASON });
  const role = guild.roles.cache.find((r) => norm(r.name) === norm(keyHolder));
  if (role) {
    await target.permissionOverwrites.edit(role, { ViewChannel: true, SendMessages: true }, { reason: PROVISION_REASON });
  } else {
    // Warned rather than passed over in silence, because a rerun does NOT heal
    // it: the channel exists now, so the next plan adopts it and emits no
    // setPermissions step. Until an operator acts, only the bot is in there.
    log.warn(
      "channel hidden, but the role that should open it does not exist — nobody but the bot can see it. " +
        "Create the role and grant it View Channel by hand (or delete the channel and reboot).",
      { channel: target.name, role: keyHolder, policy },
    );
  }
  if (botId) {
    await target.permissionOverwrites.edit(botId, { ViewChannel: true, SendMessages: true }, { reason: PROVISION_REASON });
...
async function executeStep(guild: Guild, step: ProvisionStep, created: string[], log: Logger): Promise<void> {
  switch (step.op) {
    ...
    case "createCategory": {
      const desired = DESIRED_STRUCTURE.categories.find((c) => norm(c.name) === norm(step.name));
      await guild.channels.create({
        name: step.name,
        type: ChannelType.GuildCategory,
        permissionOverwrites: creationOverwrites(desired?.overwrites, guild.roles.everyone.id, guild.client.user?.id),
        reason: PROVISION_REASON,
      });
    ...
    case "createForum": {
      ...
      created.push(`forum:${step.name}`);
      log.info("created forum channel", {
        name: step.name,
        parent: step.parentName,
        tags: tags.length,
      });
    ...
    case "setPermissions": {
      ...
      await applyOverwrites(guild, target, step.overwrites, log);
...
/** Resolve the well-known channel ids from a snapshot (empty string = not found). */
function resolveWellKnown(existing: readonly ExistingEntity[]): Omit<ProvisionDiscordResult, "created"> {
  ...
  const gallerySpotlight = findExistingChannel(
    existing,
    GALLERY_SPOTLIGHT_CHANNEL,
    "text",
    GALLERY_CATEGORY,
  );
  const pantheonArts = findExistingChannel(
    existing,
    PANTHEON_ARTS_CHANNEL,
    "text",
    PANTHEON_CATEGORY,
  );
  const pantheonTales = findExistingChannel(
    existing,
    PANTHEON_TALES_CHANNEL,
    "text",
    PANTHEON_CATEGORY,
  );
  const canon = findExistingChannel(existing, CANON_CHANNEL, "text", CANON_CATEGORY);
  const general = findExistingChannel(existing, GENERAL_CHANNEL, "text", AGORA_CATEGORY);
  const bulletin = findExistingChannel(existing, MOMUS_BULLETIN_CHANNEL, "text", BULLETIN_CATEGORY);
  return {
    announceChannelId: announce?.id ?? "",
    modLogChannelId: modLog?.id ?? "",
    welcomeChannelId: welcome?.id ?? "",
    gallerySpotlightChannelId: gallerySpotlight?.id ?? "",
    pantheonArtsChannelId: pantheonArts?.id ?? "",
    pantheonTalesChannelId: pantheonTales?.id ?? "",
    canonChannelId: canon?.id ?? "",
    generalChannelId: general?.id ?? "",
    bulletinChannelId: bulletin?.id ?? "",
  };
}
...
export async function provisionDiscord(opts: {
  ...
  try {
    ...
    return {
      ...
      pantheonArtsChannelId: "",
      pantheonTalesChannelId: "",
      canonChannelId: "",
      generalChannelId: "",
      bulletinChannelId: "",
      created,
